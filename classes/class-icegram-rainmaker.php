	...
	class Rainmaker {
		...
		function __construct() {
			...
			if ( is_admin() ) {
				...
				$ig_rm_tracker = 'IG_Tracker_V_' . str_replace( '.', '_', $feedback_version );
		...
		function execute_shortcode( $atts = array() ) {
			...
			if ( $rm_form_settings['type'] == 'custom' ) {
				...
				foreach ( $form_type_data as $key => $field ) {
					...
					if ( in_array( 'gdpr/gdpr.php', $active_plugins ) && $field['field_type'] == 'button' ) {
		...
		public static function get_rm_meta_info() {
			...
			$meta_info       = array(
				'total_leads' => $total_leads_obj->publish,
				'total_forms' => $total_forms_obj->publish,
			);
